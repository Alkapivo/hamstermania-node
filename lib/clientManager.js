...
class ClientManager {
  ...
  disconnectClient(client) {
    const destroyClientId = client.clientId;
    console.info("unregister client: " + destroyClientId);
    this.getClients()
      .forEach(client => {
        if (client.clientId != destroyClientId) {
          client.send('unregister', { removeClientId: destroyClientId });
        }
      });
  ...
  addClient(client) {
    ...
    client.on('connection_error', dc);
    client.on('connection_timeout', dc);
    client.on('end', dc);
